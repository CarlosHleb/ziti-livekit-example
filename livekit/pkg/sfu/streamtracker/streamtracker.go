...
func (s *StreamTracker) resetLocked() {
	...
	log.Print("generation resetLocked")
	s.generation.Inc()
...
func (s *StreamTracker) SetPaused(paused bool) {
	...
	if !paused {
		...
		// bump generation to trigger exit of current worker
		log.Print("generation setpaused")
		s.generation.Inc()
...
func (s *StreamTracker) worker(generation uint32) {
	...
	log.Print("worker ticker interval  ", s.params.StreamTrackerImpl.GetCheckInterval())
	log.Print("bitratereportinterval ", s.params.BitrateReportInterval)
